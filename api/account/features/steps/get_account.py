...
@then('检查 token 返回包体 res.body, ok: {ok:bool}, phone: "{phone:str}", email: "{email:str}", password: "{password:str}", firstname: "{firstname:str}", lastname: "{lastname:str}", birthday: "{birthday:str}", gender: {gender:int}')
def step_impl(context, ok, phone, email, password, firstname, lastname, birthday, gender):
    ...
    assert_that(context.res["account"]["password"], equal_to(password))
    assert_that(context.res["account"]["firstName"], equal_to(firstname))
    assert_that(context.res["account"]["lastName"], equal_to(lastname))
    assert_that(context.res["account"]["birthday"][:10], equal_to(birthday))
